...
import { serviceOrder, processOrder, whyOrder, type SiteDictionary } from '@/content'
...
export default function StudioHome({ locale, dictionary: d }: { locale: Locale; dictionary: SiteDictionary }) {
  ...
  return <div className="studio-home">
    <Header locale={locale} dictionary={d} />
    <main id="main-content">
      ...
      <section className="studio-section studio-contact" id="contact"><div className="studio-container studio-contact-grid"><div className="studio-contact-copy"><p className="studio-eyebrow">{d.navigation.contact}</p><h2>{t('הדבר הגדול הבא\nמתחיל בשיחה.', 'The next big thing\nstarts with a conversation.')}</h2><p>{d.contact.description}</p><a className="studio-contact-email" href={`mailto:${siteConfig.email}`} dir="ltr">{siteConfig.email}<Arrow /></a><div><WhatsAppAction locale={locale} /></div><span className="studio-contact-spark" aria-hidden="true">✳</span></div><div className="studio-contact-form"><ContactForm locale={locale} content={d.contact.form} /></div></div></section>
